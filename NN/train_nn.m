...
test_CE_list = zeros(1, num_epochs);
...
for epoch = 1:num_epochs
  ...
  train_CE = mean(abs(rounded - target_train));
  ...
  rounded = round(prediction);
  valid_CE = mean(abs(rounded - target_valid));
  ...
  run('test_nn.m');
  test_CE_list(1, epoch) = test_CE;
  fprintf(1,'%d  Train CE=%f, Valid CE=%f, Test CE=%f\n',...
            total_epochs, train_CE, valid_CE, test_CE);
end
